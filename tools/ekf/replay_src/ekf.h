...
static void ekf_predict(FTYPE dt, FTYPE u_alpha, FTYPE u_beta) {
    ...
    subx[0] = 1.0/J;
    subx[1] = 3*L_d - 3*L_q;
    subx[2] = 1.22474487139159*lambda_r + (1.0L/2.0L)*state[2]*subx[1];
    subx[3] = N_P*dt;
    subx[4] = cos(state[1]);
    subx[5] = sin(state[1]);
    subx[6] = (L_q*N_P*state[0]*state[3] - R_s*state[2] + subx[4]*u_alpha + subx[5]*u_beta)/L_d;
    subx[7] = (-L_d*N_P*state[0]*state[2] - 1.22474487139159*N_P*lambda_r*state[0] - R_s*state[3] + subx[4]*u_beta - subx[5]*u_alpha)/L_q;
    subx[8] = dt*subx[0];
    subx[9] = N_P*dt*subx[0]*subx[2];
    subx[10] = (1.0L/2.0L)*N_P*dt*state[3]*subx[0]*subx[1];
    subx[11] = cov[11]*subx[10] + cov[13]*subx[9] - cov[14]*subx[8] + cov[4];
    subx[12] = cov[10]*subx[10] + cov[12]*subx[9] - cov[13]*subx[8] + cov[3];
    subx[13] = cov[10]*subx[9] - cov[11]*subx[8] + cov[2] + cov[9]*subx[10];
    subx[14] = cov[0] + (1.0L/2.0L)*cov[2]*state[3]*subx[0]*subx[1]*subx[3] + cov[3]*subx[0]*subx[2]*subx[3] - cov[4]*subx[8];
    subx[15] = cov[1] + cov[6]*subx[10] + cov[7]*subx[9] - cov[8]*subx[8];
    subx[16] = 1 - R_s*dt/L_d;
    subx[17] = dt*(subx[4]*u_beta - subx[5]*u_alpha)/L_d;
    subx[18] = L_q*N_P*dt*state[0]/L_d;
    subx[19] = L_q*state[3]/L_d;
    subx[20] = 1 - R_s*dt/L_q;
    subx[21] = dt*(-subx[4]*u_alpha - subx[5]*u_beta)/L_q;
    subx[22] = dt*(-L_d*N_P*state[2] - 1.22474487139159*N_P*lambda_r)/L_q;
    subx[23] = L_d*N_P*dt*state[0]/L_q;
    subx[24] = cov[10]*subx[18] + cov[2]*subx[19]*subx[3] + cov[6]*subx[17] + cov[9]*subx[16];
    subx[25] = cov[1]*subx[19]*subx[3] + cov[5]*subx[17] + cov[6]*subx[16] + cov[7]*subx[18];
    subx[26] = cov[10]*subx[16] + cov[12]*subx[18] + cov[3]*subx[19]*subx[3] + cov[7]*subx[17];
    subx[27] = cov[0]*subx[19]*subx[3] + cov[1]*subx[17] + cov[2]*subx[16] + cov[3]*subx[18];
    state_n[0] = dt*subx[0]*(N_P*state[3]*subx[2] - state[4]) + state[0];
    state_n[1] = state[0]*subx[3] + state[1];
    state_n[2] = dt*subx[6] + state[2];
    state_n[3] = dt*subx[7] + state[3];
    state_n[4] = state[4];
    x_at_curr_meas[0] = state[0] + subx[0]*(dt - i_delay)*(N_P*state[3]*subx[2] - state[4]);
    x_at_curr_meas[1] = N_P*state[0]*(dt - i_delay) + state[1];
    x_at_curr_meas[2] = state[2] + subx[6]*(dt - i_delay);
    x_at_curr_meas[3] = state[3] + subx[7]*(dt - i_delay);
    x_at_curr_meas[4] = state[4];
    cov_n[0] = pow(omega_pnoise, 2) + subx[10]*subx[13] - subx[11]*subx[8] + subx[12]*subx[9] + subx[14];
    cov_n[1] = subx[14]*subx[3] + subx[15];
    cov_n[2] = subx[12]*subx[18] + subx[13]*subx[16] + subx[14]*subx[19]*subx[3] + subx[15]*subx[17];
    cov_n[3] = subx[12]*subx[20] - subx[13]*subx[23] + subx[14]*subx[22] + subx[15]*subx[21];
    cov_n[4] = subx[11];
    cov_n[5] = cov[1]*subx[3] + cov[5] + subx[3]*(cov[0]*subx[3] + cov[1]) + pow(theta_pnoise, 2);
    cov_n[6] = subx[16]*(cov[2]*subx[3] + cov[6]) + subx[17]*(cov[1]*subx[3] + cov[5]) + subx[18]*(cov[3]*subx[3] + cov[7]) + subx[19]*subx[3]*(cov[0]*subx[3] + cov[1]);
    cov_n[7] = subx[20]*(cov[3]*subx[3] + cov[7]) + subx[21]*(cov[1]*subx[3] + cov[5]) + subx[22]*(cov[0]*subx[3] + cov[1]) - subx[23]*(cov[2]*subx[3] + cov[6]);
    cov_n[8] = cov[4]*subx[3] + cov[8];
    cov_n[9] = N_P*dt*subx[19]*subx[27] + subx[16]*subx[24] + subx[17]*subx[25] + subx[18]*subx[26] + pow(dt, 2)*pow(subx[4], 2)*pow(u_noise, 2)/pow(L_d, 2) + pow(dt, 2)*pow(subx[5], 2)*pow(u_noise, 2)/pow(L_d, 2);
    cov_n[10] = subx[20]*subx[26] + subx[21]*subx[25] + subx[22]*subx[27] - subx[23]*subx[24];
    cov_n[11] = cov[11]*subx[16] + cov[13]*subx[18] + cov[4]*subx[19]*subx[3] + cov[8]*subx[17];
    cov_n[12] = subx[20]*(-cov[10]*subx[23] + cov[12]*subx[20] + cov[3]*subx[22] + cov[7]*subx[21]) + subx[21]*(cov[1]*subx[22] + cov[5]*subx[21] - cov[6]*subx[23] + cov[7]*subx[20]) + subx[22]*(cov[0]*subx[22] + cov[1]*subx[21] - cov[2]*subx[23] + cov[3]*subx[20]) - subx[23]*(cov[10]*subx[20] + cov[2]*subx[22] + cov[6]*subx[21] - cov[9]*subx[23]) + pow(dt, 2)*pow(subx[4], 2)*pow(u_noise, 2)/pow(L_q, 2) + pow(dt, 2)*pow(subx[5], 2)*pow(u_noise, 2)/pow(L_q, 2);
    cov_n[13] = -cov[11]*subx[23] + cov[13]*subx[20] + cov[4]*subx[22] + cov[8]*subx[21];
