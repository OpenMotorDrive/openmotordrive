...
static void ekf_predict(float dt, float u_alpha, float u_beta) {
    ...
    subx[2] = dt/J;
    subx[3] = N_P*dt;
    subx[4] = cosf_fast(state[1]);
    subx[5] = sinf_fast(state[1]);
    subx[6] = (L_q*N_P*state[0]*state[3] - R_s*state[2] + subx[4]*u_alpha + subx[5]*u_beta)/L_d;
    subx[7] = (-L_d*N_P*state[0]*state[2] - R_s*state[3] - 36.7423461417477*state[0]*subx[1] + subx[4]*u_beta - subx[5]*u_alpha)/L_q;
    subx[8] = cov[11]*state[3]*subx[0]*subx[2] + cov[13]*subx[2]*(state[2]*subx[0] + 30.0*subx[1]) - cov[14]*subx[2] + cov[4];
    subx[9] = dt*(state[2]*subx[0] + 30.0*subx[1])/J;
    subx[10] = dt*state[3]*subx[0]/J;
    subx[11] = cov[10]*subx[10] + cov[12]*subx[9] - cov[13]*subx[2] + cov[3];
    subx[12] = cov[10]*subx[9] - cov[11]*subx[2] + cov[2] + cov[9]*subx[10];
    subx[13] = cov[0] + cov[2]*subx[10] + cov[3]*subx[9] - cov[4]*subx[2];
    subx[14] = cov[1] + cov[6]*subx[10] + cov[7]*subx[9] - cov[8]*subx[2];
    subx[15] = 1 - R_s*dt/L_d;
    subx[16] = dt*(subx[4]*u_beta - subx[5]*u_alpha)/L_d;
    subx[17] = L_q*N_P*dt*state[0]/L_d;
    subx[18] = L_q*state[3]/L_d;
    subx[19] = 1 - R_s*dt/L_q;
    subx[20] = dt*(-subx[4]*u_alpha - subx[5]*u_beta)/L_q;
    subx[21] = dt*(-L_d*N_P*state[2] - 36.7423461417477*subx[1])/L_q;
    subx[22] = L_d*N_P*dt*state[0]/L_q;
    subx[23] = cov[10]*subx[17] + cov[2]*subx[18]*subx[3] + cov[6]*subx[16] + cov[9]*subx[15];
    subx[24] = cov[1]*subx[18]*subx[3] + cov[5]*subx[16] + cov[6]*subx[15] + cov[7]*subx[17];
    subx[25] = cov[10]*subx[15] + cov[12]*subx[17] + cov[3]*subx[18]*subx[3] + cov[7]*subx[16];
    subx[26] = cov[0]*subx[18]*subx[3] + cov[1]*subx[16] + cov[2]*subx[15] + cov[3]*subx[17];
    state_n[0] = state[0] + subx[2]*(state[2]*state[3]*subx[0] + 30.0*state[3]*subx[1] - state[4]);
    state_n[1] = state[0]*subx[3] + state[1];
    state_n[2] = dt*subx[6] + state[2];
    state_n[3] = dt*subx[7] + state[3];
    state_n[4] = state[4];
    x_at_curr_meas[0] = state[0] + (dt - i_delay)*(state[2]*state[3]*subx[0] + 30.0*state[3]*subx[1] - state[4])/J;
    ...
    cov_n[0] = ((omega_pnoise)*(omega_pnoise)) + subx[10]*subx[12] + subx[11]*subx[9] + subx[13] - subx[2]*subx[8];
    cov_n[1] = subx[13]*subx[3] + subx[14];
    cov_n[2] = subx[11]*subx[17] + subx[12]*subx[15] + subx[13]*subx[18]*subx[3] + subx[14]*subx[16];
    cov_n[3] = subx[11]*subx[19] - subx[12]*subx[22] + subx[13]*subx[21] + subx[14]*subx[20];
    cov_n[4] = subx[8];
    cov_n[5] = cov[1]*subx[3] + cov[5] + subx[3]*(cov[0]*subx[3] + cov[1]) + ((theta_pnoise)*(theta_pnoise));
    cov_n[6] = subx[15]*(cov[2]*subx[3] + cov[6]) + subx[16]*(cov[1]*subx[3] + cov[5]) + subx[17]*(cov[3]*subx[3] + cov[7]) + subx[18]*subx[3]*(cov[0]*subx[3] + cov[1]);
    cov_n[7] = subx[19]*(cov[3]*subx[3] + cov[7]) + subx[20]*(cov[1]*subx[3] + cov[5]) + subx[21]*(cov[0]*subx[3] + cov[1]) - subx[22]*(cov[2]*subx[3] + cov[6]);
    cov_n[8] = cov[4]*subx[3] + cov[8];
    cov_n[9] = N_P*dt*subx[18]*subx[26] + subx[15]*subx[23] + subx[16]*subx[24] + subx[17]*subx[25] + ((dt)*(dt))*((subx[4])*(subx[4]))*((u_noise)*(u_noise))/((L_d)*(L_d)) + ((dt)*(dt))*((subx[5])*(subx[5]))*((u_noise)*(u_noise))/((L_d)*(L_d));
    cov_n[10] = subx[19]*subx[25] + subx[20]*subx[24] + subx[21]*subx[26] - subx[22]*subx[23];
    cov_n[11] = cov[11]*subx[15] + cov[13]*subx[17] + cov[4]*subx[18]*subx[3] + cov[8]*subx[16];
    cov_n[12] = subx[19]*(-cov[10]*subx[22] + cov[12]*subx[19] + cov[3]*subx[21] + cov[7]*subx[20]) + subx[20]*(cov[1]*subx[21] + cov[5]*subx[20] - cov[6]*subx[22] + cov[7]*subx[19]) + subx[21]*(cov[0]*subx[21] + cov[1]*subx[20] - cov[2]*subx[22] + cov[3]*subx[19]) - subx[22]*(cov[10]*subx[19] + cov[2]*subx[21] + cov[6]*subx[20] - cov[9]*subx[22]) + ((dt)*(dt))*((subx[4])*(subx[4]))*((u_noise)*(u_noise))/((L_q)*(L_q)) + ((dt)*(dt))*((subx[5])*(subx[5]))*((u_noise)*(u_noise))/((L_q)*(L_q));
