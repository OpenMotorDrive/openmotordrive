...
{
    if (!phase_output[phase_output_idx].enable) {
        drv_3_pwm_mode(void);
    }

    uint8_t next_phase_output_idx = (phase_output_idx+1)%2;
    phase_output[next_phase_output_idx].enable = true;
...
void inverter_disable_output()
{
    uint8_t next_phase_output_idx = (phase_output_idx+1)%2;
    phase_output[next_phase_output_idx].enable = false;
...
{
    struct adc_sample_s* adc_sample = adc_get_sample();
    ...
    // Use the two current sensors on the phases with the lowest duty cycles
    float duty_a, duty_b, duty_c;
    pwm_get_phase_duty(&duty_a, &duty_b, &duty_c);

    if (duty_a > duty_b && duty_a > duty_c) {
        sense_data[next_sense_data_idx].i_a = -motor_state.i_b-motor_state.i_c;
    } else if (duty_b > duty_a && duty_b > duty_c) {
        sense_data[next_sense_data_idx].i_b = -motor_state.i_a-motor_state.i_c;
    } else {
        sense_data[next_sense_data_idx].i_c = -motor_state.i_a-motor_state.i_b;
    }
...
static void new_adc_data(void)
{
    ...
    uint32_t dt_us = sense_data[sense_data_idx].t_us-phase_output[phase_output_idx].t_us + 1e6f*pwm_get_period()/2;
    ...
    alpha /= sense_data.v_bus;
    beta /= sense_data.v_bus;
